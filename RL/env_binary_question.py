...
class BinaryRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, seed=1, max_turn=15, cand_len_size=20, attr_num=20, mode='train', command=1, ask_num=1, entropy_way='weight entropy', fm_epoch=0):
        ...
        self.cur_node_set = []     #maybe a node or node set  /   normally save feature node
        # state veactor
    ...
    def _get_state(self):
        ...
        if self.command == 1:
            ...
            # print(' attr_ent:', self.attr_ent)
            print('lastfm_large env3 cand_len to Decimal num:', len(self.cand_items))
    ...
    def step(self, action):   #action:0  ask   action:1  recommend   setp=MAX_TURN  done
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            # TODO update_feature_entropy!  reject_feature
            ...
            print('--topK item score:',cand_item_score[:10])
            ...
            if reward == 1:
                ...
                print('--new topK item score:', cand_item_score[10:20])
    ...
    def _updata_reachable_feature(self, start='feature'):
        ...
        if start == 'user':
            ...
            # user_like_fea = list(self.kg.G['user'][self.user_id]['like'])  # U-A
            # self.reachable_feature.append(user_like_fea)
            #
            # user_friends = list(self.kg.G['user'][self.user_id]['friends'])  #U-U
            # user_listen_items = list(self.kg.G['user'][self.user_id]['interact']) #U-I
            # #update reachable feature
            # if self.data_name in ['lastfm']:
            #     for user_id in user_friends:   # U-U-A
            #         self.reachable_feature.append(list(self.kg.G['user'][user_id]['like']))
            #     # change list into 1-dim list   &  remove redundant features
            #     self.reachable_feature = list(set(_flatten(self.reachable_feature)))
            #
            # for item_id in user_listen_items:  #U-I-A
            #     self.reachable_feature.append(list(self.kg.G['item'][item_id]['belong_to']))
            # self.reachable_feature = list(set(_flatten(self.reachable_feature)))
        elif start == 'feature':
            next_reachable_feature = []
            for cur_node in self.cur_node_set:
    ...
    def _ask_update(self):
        ...
        #TODO ask new feature that not in accpeted set * rejected set !!
        remove_acced_reachable_fea = self.reachable_feature.copy()  # copy reachable_feature
        # for fea_id in self.user_acc_feature:
        #     if fea_id in remove_acced_reachable_fea:
        #         remove_acced_reachable_fea.remove(fea_id)
        # for fea_id in self.user_rej_feature:
        #     if fea_id in remove_acced_reachable_fea:
        #         remove_acced_reachable_fea.remove(fea_id)
        ...
        # print(' sorted reachable feature :', self.reachable_feature)
        # print(' feature ent:', self.attr_ent)
        print(' sorted reachable feature num :', len(self.reachable_feature))
        # print(' sorted reachable feature top K:', self.reachable_feature[:self.ask_num])
        # print(' sorted reachable feature top K(remove acc_rej set)', remove_acced_reachable_fea[:self.ask_num])
    ...
    def _update_cand_items(self, acc_feature, rej_feature):  # TODO： YELP large tag need to modify
        ...
        # if len(rej_feature):  #reject feature
        #     print('=== ask rej: update cand_items')
        #     for feature_id in rej_feature:
        #         neg_feature_items = self.kg.G['feature'][feature_id]['belong_to']
    ...
    def _update_feature_entropy(self):  #YELP need to modify
        if self.ent_way == 'entropy':
            ...
            # updata  state_vector: self.attr_ent
            self.attr_ent = [0] * self.attr_state_num  # reset attr_ent
            # remove_acc_rej_reachable_feature = list(set(self.reachable_feature) - set(self.user_acc_feature))
            # remove_acc_rej_reachable_feature = list(set(remove_acc_rej_reachable_feature) - set(self.user_rej_feature))
            ...
            print('sum score sig:', sum_score_sig)
            for fea_id in real_ask_able:
